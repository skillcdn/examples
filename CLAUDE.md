...
- Front-matter is parsed by a real YAML parser on the server, and a value it cannot parse drops the whole `SKILL.md` silently: the mount then reports no skills at all. The usual cause is a colon followed by a space in an unquoted `description`. Rephrase or quote; `scripts/check.mjs` rejects it.
