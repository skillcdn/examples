...
This repository holds the skills SkillCDN serves as an MCP server at `skillcdn.ai/gh/skillcdn/skills`: skills for everyday work, one folder per area (marketing, product, engineering, more to come), each driving a real tool. It is the reference repository for the [SkillCDN Format](https://github.com/skillcdn/skillcdn/blob/main/docs/specs/skill-repo.md): the shape SkillCDN proposes for a skill repository. It takes no pull requests; it is meant to be forked, so that a team publishes its own skills in the same shape with the same checks. Content, not code: Markdown that agents read, small JSON, and one dependency-free script that validates it. Every push to `main` is a public release, because SkillCDN indexes the default branch.
...
- Discoverable and readable are different. `browse` and `search` see skills, manifests and declared document directories; a README in a served folder, and any page it links, can be read with `read_file` but is never searched or paged into a skill. A page that must be searchable goes under a `docs/`; authoring material goes under `guide/`.
...
- Rules are paged before the skill. `get_skill` returns the root manifest's body, then the area's, then the skill and its included files, in 16 KiB pages. Keep an area's rules to a few lines; a long body costs every skill in the area a page.
